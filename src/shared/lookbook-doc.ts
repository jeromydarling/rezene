 ...
 * `@page`. Shared by the client (live preview + Save-as-PDF) and the worker
 * (the render pipeline captures it headlessly for a print-on-demand fulfiller).
 *
 * `opts.part` splits the document for a saddle-stitch print job:
 *   - "full"     → the whole magazine (default; used for preview + Save-as-PDF)
 *   - "cover"    → the front cover only
 *   - "interior" → everything except the front cover (opener + spreads + back)
 */
import { brandFont, applyCase } from "./brand-identity";
import type { LookbookBrand, LookbookLayout, LookbookProduct, LookbookRenderModel } from "./lookbook";
...
function logoHtml(brand: LookbookBrand, onDark: boolean, px: number): string {
...
function pieceCaption(p: LookbookProduct, brand: LookbookBrand, opts: { detail: boolean; onDark?: boolean }): string {
...
function imageBox(p: LookbookProduct, brand: LookbookBrand, style: string): string {
  if (p.imageUrl) {
...
function spreadPage(p: LookbookProduct, layout: LookbookLayout, caption: string, brand: LookbookBrand): string {
...
function coverPage(model: LookbookRenderModel, brand: LookbookBrand): string {
...
function introPage(model: LookbookRenderModel, brand: LookbookBrand): string {
  const lb = model.lookbook;
...
function backPage(brand: LookbookBrand): string {
...
/** Build the print-ready magazine document (or one part of it). */
export function buildLookbookDoc(
  model: LookbookRenderModel,
  brand: LookbookBrand,
  opts: { print?: boolean; part?: "full" | "cover" | "interior" } = {},
): string {
  ...
  const cover = coverPage(model, brand);
  const rest = [
    introPage(model, brand),
    ...model.spreads.map((s) => spreadPage(s.product, s.layout, s.caption, brand)),
    backPage(brand),
  ].join("\n");
  const pages = part === "cover" ? cover : part === "interior" ? rest : `${cover}\n${rest}`;
