...
**Markdowns are the tax on misreading fashion.** Nystrom's merchandising rule: goods whose value is fashion-driven lose value as acceptance passes, on a schedule no one can suspend. The discipline is to buy (or cut) shallow on the fashion-driven part of a line, deep on the style-driven part — and to take the first markdown early, when it is small, rather than late, when it is fatal. Pre-orders, small-batch runs and made-to-measure — the modern independent's toolkit — are all ways of pushing that risk toward zero. Your ancestors in this book managed it with open-to-buy budgets, which is why Verto has one.`,
      checkpoints: [
        {
