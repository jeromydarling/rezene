...
**True-drape preview (beta).** For a growing set of blocks — **t-shirt, tank, sweatshirt, raglan hoodie, the Simon/Simone button-downs, zip-up hoodie, waistcoat, slip dress, trousers (block and summer pants), pencil skirt, circle skirt, and the Bella bodice block** — *Simulate the real drape* sews your exact draft together in a cloth simulator and drapes it on a ghost mannequin **scaled to the same measurements you drafted with**. The result is a physically true picture of the proportions: where the hem really sits, how far the sleeves really reach, how much ease really hangs at the waist. It takes a few minutes on a render server; when it's done, **Render on a model from this drape** feeds the simulated garment to the photoreal engine as a proportion reference — so the picture your client sees matches the pattern your cutter holds. Alongside the drape you also get a **fit map** — the same view professional 3D fit tools use: green where the fabric skims comfortably, yellow where it's snug, red where it's pulling tight against your pattern (exactly where a real toile would show drag lines), and blue where slack is pooling. It's measured against the flat pattern itself, in your client's measurements. Since patterns are drafted *to* those measurements, most drafts sit green and blue — red is the interesting signal, appearing where a client's proportions genuinely exceed what the block's shaping allows, or when you've pushed ease toward zero. A fitting's worth of information before any fabric is cut. Honest scope: hoods, collars and waistbands aren't simulated (your description carries those), and the sim's grey surface is deliberately rough — the render engine is told to read proportions from it, never texture.
